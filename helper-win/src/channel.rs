...
pub struct Channel {
    wts: HANDLE,
    file: HANDLE,
    /// Manual-reset event of the overlapped calls: one call is in flight at a time
    event: HANDLE,
    buffer: Vec<u8>,
}
...
impl Channel {
    ...
    pub fn open() -> io::Result<Channel> {
        ...
        channel.file = file_handle(wts)?;
        // SAFETY: plain event creation, no name and no security attributes
        channel.event = unsafe { CreateEventW(null(), 1, 0, null()) };
        if channel.event.is_null() {
            return Err(io::Error::last_os_error());
        }
        Ok(channel)
    }
    ...
    pub fn read(&mut self) -> io::Result<&[u8]> {
        ...
        let read = self.finish(started, &overlapped)?;
    ...
    pub fn write(&mut self, bytes: &[u8]) -> io::Result<()> {
        ...
        let written = self.finish(started, &overlapped)?;
    ...
    fn finish(&self, started: i32, overlapped: &OVERLAPPED) -> io::Result<usize> {
        ...
        if unsafe { GetOverlappedResult(self.file, overlapped, &mut moved, 1) } == 0 {
            return Err(io::Error::last_os_error());
        }
        Ok(moved as usize)
    }
...
impl Drop for Channel {
    fn drop(&mut self) {
        // SAFETY: each handle is owned here and closed once; null ones were never opened
        unsafe {
