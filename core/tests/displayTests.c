...
static void setUp(VRCDisplay* display)
{
    ...
    vrcDisplayAttach(display, &channel, 1024, 640);
}
...
{
    DISPLAY_CONTROL_MONITOR_LAYOUT layout = vrcDisplayLayout(1921, 1199);
...
static bool testWaitsForCapabilities(void)
{
    VRCDisplay display;
    setUp(&display);
    vrcDisplayRequest(&display, 1920, 1200);
    CHECK(layouts == 0);
...
static bool testOnlyNewSizesGo(void)
{
    ...
    CHECK(channel.DisplayControlCaps(&channel, 16, 8192, 8192) == CHANNEL_RC_OK);
    CHECK(layouts == 0);
    vrcDisplayRequest(&display, 1024, 640);
    CHECK(layouts == 0);
    vrcDisplayRequest(&display, 1281, 800);
    vrcDisplayRequest(&display, 1280, 800);
    CHECK(layouts == 1 && last.Width == 1280);
    vrcDisplayRequest(&display, 1440, 900);
