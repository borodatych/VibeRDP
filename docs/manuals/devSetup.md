...
Тесты `clipboardTests` гоняют протокол буфера обмена на поддельном канале, `cliptextTests` — преобразования текста, а `clipboardEchoTests` отправляют текст через sample-сервер, который возвращает его с приставкой.
