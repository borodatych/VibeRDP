...
Тесты `clipboardTests` гоняют протокол буфера обмена на поддельном канале, `cliptextTests` и `cliphtmlTests` — преобразования текста и HTML, а `clipboardEchoTests` отправляют текст, HTML и RTF через sample-сервер, который возвращает их.
