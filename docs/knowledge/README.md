...
- [sampleServer.md](freerdp/sampleServer.md) — sample-сервер как собеседник живого теста: копия исходников с патчами, `--local-only` — Unix-сокет в `TMPDIR`, иконка из рабочей папки, запись `rfx_test.pcap` кадр за кадром и точка, синяя во всех кадрах; дефект проигрыша — хвост прошлой записи в каждом пакете, исправлен патчем
