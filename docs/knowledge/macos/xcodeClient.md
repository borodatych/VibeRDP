...
- С задачи 1.4 Xcode 26.6 в CI ссылается на `_dispatch_once_f` слабо, а Xcode 27 то же приложение линкует со ссылкой сильной; функция есть в libdispatch с его появления в macOS 10.6 и объявлена в `dispatch/once.h` без ограничения версии, так что ссылка разрешается на любой поддерживаемой системе
- _Не проверено:_ что именно в тулчейне 26.6 делает её слабой — здесь стоит только Xcode 27, а в библиотеках совместимости Swift 27 ссылка сильная

**Применение:** `check_binary` в `core/scripts/common.sh` пропускает именно эти символы — список `TOOLCHAIN_WEAK_SYMBOLS`, символы Swift по префиксу `_$s` и `_dispatch_once_f`; любая другая слабая ссылка, из C и Objective-C в первую очередь, роняет сборку.
...
## Тесты внутри приложения запускают приложение целиком

**Суть:**
- Тесты клиента идут внутри `VibeRDP.app`: `applicationDidFinishLaunching` отрабатывает у каждого запуска тестов, и всё, что приложение делает при старте с папками пользователя, делает и прогон
- Журнал диагностики так вытеснял журналы владельца: хранятся десять последних, а один прогон `build-client.sh` запускает приложение трижды — 2026-09-26 три прогона чуть не стёрли журнал с обрывами, по которому искали дефект
- XCTest передаёт процессу-хосту переменную `XCTestConfigurationFilePath`; у запуска из Finder и из `open` её нет
- Проверено: полный прогон 161 теста на двух архитектурах после правки не добавил в `~/VibeRDP/logs/` ни одного файла

**Применение:**
- `AppDelegate` не открывает журнал, когда задана `XCTestConfigurationFilePath`; журнал проверяют `DiagnosticsTests` на своей временной папке
- Засев папки языков при старте остаётся и в прогоне: он не удаляет чужого — [localization.md](localization.md)
