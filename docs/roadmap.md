...
- [ ] 0.5 `helper-win` — cargo-проект, пустой exe, сборка в CI — проект готов: exe без консоли, C-рантайм внутри (`+crt-static`), тулчейн 1.97.1 закреплён; rustfmt и clippy чисты под хост и под `x86_64-pc-windows-msvc`; задача CI `helper` собирает exe на `windows-2025` и проверяет его `helper-win/scripts/check-exe.ps1`; **осталось увидеть первый прогон** — репозитория на GitHub ещё нет
- [ ] 0.6 GitHub Actions: сборка клиента (macOS) и хелпера (Windows) на каждый push — `.github/workflows/ci.yml` написан и проверен actionlint: macOS-задача собирает зависимости, ядро и клиент с тестами, Windows — exe хелпера, линт — shellcheck и actionlint закреплённых версий — [решение 13](decisions.md), [githubActions.md](knowledge/ci/githubActions.md); **осталось увидеть первый прогон** — репозитория на GitHub ещё нет
...
- [ ] 1.9 Локализация: языки файлами в папке, которую видит пользователь, выбор языка, откат пропавшего языка, гейт каталога — по правилу семейства; база с 0.4 — русский в бинаре (`client-macos/App/Localization.swift`) — _добавлено вне спецификации, 2026-09-24_
